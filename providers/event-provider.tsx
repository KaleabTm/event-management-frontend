...
export function EventProvider({ children }: { children: ReactNode }) {
  ...
  const getEventsForDate = (date: string): Event[] => {
    return events.filter((event) => {
      ...
      return event.startDate === date || (event.recurrence.type !== "none" && isRecurringEventOnDate(event, date))
    })
  }
  ...
  const isRecurringEventOnDate = (event: Event, date: string): boolean => {
    ...
    switch (event.recurrence.type) {
      ...
      case "weekly":
        ...
        return weeksDiff % (event.recurrence.interval || 1) === 0 && checkDate.getDay() === eventDate.getDay()
      case "monthly":
        return checkDate.getDate() === eventDate.getDate()
      case "yearly":
        return checkDate.getMonth() === eventDate.getMonth() && checkDate.getDate() === eventDate.getDate()
      default:
        return false
    }
  }
