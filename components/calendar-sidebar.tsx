...
export default function CalendarSidebar() {
  ...
  const { calendars, fetchCalendars, toggleCalendarVisibility, deleteCalendar, getEventsByCalendar } = useEventStore()
  ...
  const handleExportCalendar = (calendar: any) => {
    const events = getEventsByCalendar(calendar.id)
    if (events.length === 0) {
        ...
        {calendars.map((calendar) => (
          <div key={calendar.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 group">
            <div className="flex items-center space-x-3 flex-1">
              <Checkbox checked={calendar.isVisible} onCheckedChange={() => toggleCalendarVisibility(calendar.id)} />
              <div className="flex items-center space-x-2 flex-1">
                ...
                <Badge variant="secondary" className="text-xs">
                  {getEventsByCalendar(calendar.id).length}
                </Badge>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              >
                {calendar.isVisible ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}
              </Button>
